...
#
def array2raster(srcImg, newRaster, listOfArrays, reductionBounds=False, trainingSelection=False, patch_size=0, pixX=0,
                 pixY=0):
    ds_raster = gdal.Open(srcImg)
    ...
    if reductionBounds:
        ...
        outRaster = driver.Create(newRaster, cols, rows, len(listOfArrays), gdal.GDT_Byte)
        ...
        cols = patch_size
        rows = patch_size
        outRaster = driver.Create(newRaster, cols, rows, len(listOfArrays), gdal.GDT_Int16)
        outRaster.SetGeoTransform(( \
